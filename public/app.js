...
function formatQualityGroupSetNote(planText = "") {
  ...
  if (isQualityTimeTrialPlan(planText) || baseSetCount <= 1) return "";

  return `${QUALITY_AUTO_REDUCED_SET_GROUPS.map((group) => `${group}조`).join(", ")}: ${baseSetCount - 1}세트`;
}

function getQualityBaseSetCountFromPlan(planText = "") {
  const match = String(planText || "").match(/(\d+(?:\.\d+)?)\s*(k|km|m)?\s*(?:x|×)\s*(\d+)/i);
...
function formatQualityHostGroupPlanGuide(planText = "") {
  if (isQualityTimeTrialPlan(planText)) {
    return `<div class="quality-plan-meta">${getQualityTimeTrialPaceGuide(planText)}</div>`;
  }

  const rows = runningGroupStandards.map((standard) => {
    const setCount = getQualitySetCountForGroup(planText, standard.group);
    const setLabel = setCount ? `${setCount}세트` : "-";

    return [
      "<tr>",
      `<td>${escapeHtml(standard.group)}조</td>`,
      `<td>${escapeHtml(standard.intervalPace)}</td>`,
      `<td>${escapeHtml(standard.recoveryPace)}</td>`,
  ...
  return [
    ...
    '<div class="quality-host-plan-title">호스트 전체 조 안내</div>',
    '<div class="quality-host-plan-table-wrap">',
    '<table class="quality-host-plan-table">',
    '<thead><tr><th>조</th><th>인터벌</th><th>리커버리</th><th>세트</th></tr></thead>',
    `<tbody>${rows}</tbody>`,
    '</table>',
    '</div>',
    '</div>'
...
function syncQualityPlanSelection({ selectedValue = "", resetResults = false, useTodayDate = false } = {}) {
  const qualityDateInput = document.getElementById("qualityDate");
  const qualityPlanSelect = document.getElementById("qualityPlanSelect");
  const qualityWorkoutTypeSelect = document.getElementById("qualityWorkoutType");
  const qualityPlannedWorkoutInput = document.getElementById("qualityPlannedWorkout");
  ...
  if (useTodayDate && qualityDateInput && !editingQualityRun) {
...
function renderRunList() {
  ...
  visibleRuns.forEach((run) => {
    ...
    actionTd.className = "member-run-action-cell";
    const actionWrap = document.createElement("div");
    actionWrap.className = "member-run-action-stack";
...
async function loadMonthlyAthleteCandidates(user) {
  ...
  try {
    ...
    try {
      const monthlyGoalSnapshot = await getDocsFromServer(collection(db, "monthlyGoals"));
      monthlyGoalSnapshot.forEach((goalDoc) => {
        const data = goalDoc.data();
        const goalMapKey = getMonthlyGoalMapKey(data.month || "", data.userId || "", data.email || "");
        const goalKm = Number(data.goalKm || 0);
...
function getAthleteHallAchievementSummary(entry) {
  ...
  if (entry.goalKm && entry.goalScore > 0) {
...
function getMonthlyGoalLookupKey(userId = "", email = "") {
  return userId || email || "";
}

function getMonthlyGoalMapKey(monthKey = "", userId = "", email = "") {
  const lookupKey = getMonthlyGoalLookupKey(userId, email);
  return monthKey && lookupKey ? `${monthKey}:${lookupKey}` : "";
}

function getMonthlyGoalTargetDistance(goalKm, monthKey, useElapsedProgress = false) {
  if (!goalKm) return 0;

  if (!useElapsedProgress) return goalKm;

  const progress = getMonthProgressInfo(monthKey);
  return progress.isCurrentMonth
    ? goalKm * (progress.elapsedDays / progress.totalDays)
    : goalKm;
}

function getMonthlyGoalRate(distance, goalKm, monthKey, useElapsedProgress = false) {
  const targetDistance = getMonthlyGoalTargetDistance(goalKm, monthKey, useElapsedProgress);
...
function getAttendanceScore(attendanceDays, maxScore = 25, useLinear = false) {
  return getLinearScore(attendanceDays, 24, maxScore);
}
